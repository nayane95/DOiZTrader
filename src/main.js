...
// ============================================
async function init() {
  console.log('Initializing OrderBook Pro...');
  ...
  console.log('OrderBook Pro is ready.');
}
...
function applySettings(settings) {
  ...
  if (chart?.setCandlePalette) {
    chart.setCandlePalette(settings.candleUpColor, settings.candleDownColor);
  }
...
function initEventHandlers() {
  ...
  document.querySelectorAll('.timeframe-btn').forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (tf !== state.timeframe) {
        ...
        // Reconnect so the websocket kline stream matches the selected timeframe.
        await connect();
      }
    });
  });
  ...
  document.getElementById('indicators-btn')?.addEventListener('click', () => {
    showToast('Indicators will be added soon.', 'info');
  });
...
function setupWebSocketHandlers() {
  ...
  wsManager.on('trade', ({ data }) => {
    const tradeVolume = Number(data.volume ?? data.qty);
    const tradePrice = Number(data.price);

    if (!Number.isFinite(tradeVolume) || tradeVolume <= 0 || !Number.isFinite(tradePrice)) {
      return;
    }
    ...
    const normalizedTrade = {
      ...
      volume: tradeVolume,
      qty: tradeVolume,
      isBuy: data.isBuy ?? data.isBuyerMaker === false
    };

    analytics.addTrade(normalizedTrade);
    state.currentPrice = normalizedTrade.price;

    if (chart) {
      chart.addTrade(normalizedTrade);
    }
    ...
    if (cvdChart) {
      cvdChart.addTrade({
        ...
        side: normalizedTrade.isBuy ? 'buy' : 'sell',
  ...
  wsManager.on('connected', ({ id, exchange, symbol }) => {
    console.log(`Connected: ${exchange} ${symbol}`);
    hideLoading();
    showToast(`Connected to ${exchange}`, 'success');
  });
...
async function loadOrderBookSnapshot() {
  try {
    ...
    if (snapshot && snapshot.bids?.length && snapshot.asks?.length) {
      ...
      console.log('Order book snapshot loaded:');
      ...
      console.log(`   Total range: ${minBidPrice.toFixed(2)} - ${maxAskPrice.toFixed(2)}`);
    ...
    console.error('Failed to load order book snapshot:', e);
  }
}
...
function switchOrderBookMode(mode) {
  ...
  if (heatmap) {
    const overlayEnabled = document.getElementById('heatmap-btn')?.classList.contains('active') ?? false;
    heatmap.setVisible(overlayEnabled);
    if (overlayEnabled) {
      heatmap.setOpacity(mode === 'heatmap' ? 0.55 : 0.35);
    }
  }
}
...
function handleMobileTabSwitch(tab) {
  ...
  switch (tab) {
    ...
    case 'draw':
      // TODO: Open drawing tools
      showToast('Drawing tools will be added soon.', 'info');
      break;
    ...
    infoPanel.innerHTML = `
      ...
      <div class="info-panel__content">
        ...
        <div class="info-panel__body">
          <div class="info-item">
            <h3>Index α (Alpha)</h3>
            <p>Measures buyer strength versus seller strength. Greater than 1 means buyers are stronger, less than 1 means sellers are stronger.</p>
          </div>
          <div class="info-item">
            <h3>Delta</h3>
            <p>The difference between aggressive buy volume and sell volume. Positive means buyers are more active, negative means sellers are more active.</p>
          </div>
          <div class="info-item">
            <h3>CVD (Cumulative Volume Delta)</h3>
            <p>The running sum of buy minus sell volume over time. It highlights whether buyer or seller pressure is building.</p>
          </div>
          <div class="info-item">
            <h3>Imbalance</h3>
            <p>Shows the order book imbalance. Positive means bids dominate, negative means asks dominate.</p>
          </div>
          <div class="info-item">
            <h3>Spread</h3>
            <p>The difference between the best ask and best bid. A smaller spread usually means better liquidity.</p>
          </div>
          <div class="info-item">
            <h3>Heatmap</h3>
            <p>Green zones represent bids, red zones represent asks, and brighter areas indicate larger resting volume.</p>
          </div>
        </div>
      </div>
    `;
