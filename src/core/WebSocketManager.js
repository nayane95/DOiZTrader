...
export class WebSocketManager {
   ...
   */
  connect(exchange, symbol, streams = ['kline', 'depth', 'trade'], options = {}) {
    ...
    const adapter = new AdapterClass(symbol, streams, options);
    ...
    adapter.onError = (error) => {
      ...
      this._scheduleReconnect(exchange, symbol, streams, options);
    };
   ...
   * @private
   */
  _scheduleReconnect(exchange, symbol, streams, options = {}) {
    const id = `${exchange}:${symbol}`;
    ...
    setTimeout(() => {
      ...
      this.connect(exchange, symbol, streams, options);
